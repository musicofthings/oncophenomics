    ...
    <aside className="hidden md:flex flex-col w-64 h-full border-r border-slate-200 dark:border-slate-800 bg-surface-light dark:bg-surface-dark shrink-0 z-50 font-display">
      ...
      <div className="flex-1 py-6 flex flex-col gap-1 px-3 overflow-y-auto">
        {[
          ...
          { icon: 'biotech', path: '/platform', label: 'Platform' },
        ].map(item => (
          <button key={item.path} onClick={() => navigate(item.path)}
            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm text-left
              ${isActive(item.path) ? 'bg-primary/10 text-primary' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white'}`}>
        ...
        <div className="mt-1">
          <button onClick={() => setExpandVariant(!expandVariant)}
            className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm text-left group
              ${location.pathname.startsWith('/variant-gpt') ? 'bg-primary/10 text-primary' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white'}`}>
            <span className={`material-symbols-outlined text-[22px] ${location.pathname.startsWith('/variant-gpt') ? 'filled' : ''}`}>smart_toy</span>
              ...
              {[
                ...
                <button key={item.path} onClick={() => navigate(item.path)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 font-medium text-sm text-left
                    ${isActive(item.path) ? 'text-primary' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}>
                  <span>{item.label}</span>
                </button>
              ))}
            </div>
        ...
        {/* Services group */}
        <div className="mt-2">
          <button onClick={() => setExpandServices(!expandServices)}
            ...
            <span className="material-symbols-outlined text-[16px] transition-transform duration-200"
              style={{ transform: expandServices ? 'rotate(0deg)' : 'rotate(-90deg)' }}>expand_more</span>
          </button>
          ...
          <div className={`flex flex-col gap-1 overflow-hidden transition-all duration-300 ${expandServices ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
            ...
            <div>
              <button onClick={() => setExpandDiagnostics(!expandDiagnostics)}
              ...
              {expandDiagnostics && (
                <div className="flex flex-col gap-1 ml-6 pl-3 border-l border-slate-200 dark:border-slate-700 mt-1">
                  {[
                    { label: 'Tissue Biopsy',     path: '/diagnostics',   activeClass: 'text-primary bg-primary/10' },
                    { label: 'Liquid Biopsy',     path: '/liquid-biopsy', activeClass: 'text-secondary bg-secondary/10' },
                    { label: 'Test Request Form', path: '/test-request',  activeClass: 'text-primary bg-blue-50 dark:bg-blue-900/10' },
                  ].map(item => (
                    <button key={item.path} onClick={() => navigate(item.path)}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 font-medium text-sm text-left
                        ${isActive(item.path) ? item.activeClass : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                      <span>{item.label}</span>
                    </button>
                  ))}
                </div>
